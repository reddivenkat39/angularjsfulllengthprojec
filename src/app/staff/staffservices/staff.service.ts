...
export class StaffService {
  ...
  sendLoginCredentials(staffLogin) {
    ...
    return this.http.post("http://localhost:8080:8080/staff/login", bodySend, {headers: headersSend})
      .map((data: Response)=> {
  ...
  sendToken(token) {
    let tokenUrl2 = "http://localhost:8080:8080/staff/staffids";
  ...
  onForgotPasswordSubmit(forgotPasswordData) {
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("http://localhost:8080:8080/staff/forgotpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
  ...
  findEmailAddressByToken(token){
     let tokenUrl2 = "http://localhost:8080:8080/findemailidbytoken";
    console.log('Bearer ' + token);
    const getHeaders = new Headers({'authorization': 'Bearer '+token});
  ...
  sendResetPasswordCredentials(forgotPasswordCredentials){
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("http://localhost:8080:8080/staff/resetpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
        console.log("sendLoginCredentials: Response ", data.json());
        return data.json();

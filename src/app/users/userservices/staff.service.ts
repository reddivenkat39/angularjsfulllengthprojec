...
export class StaffService {
  ...
  sendLoginCredentials(staffLogin) {
    ...
    return this.http.post("/tep/staff/login", bodySend, {headers: headersSend})
      .map((data: Response)=> {
  ...
  sendToken(token) {
    let tokenUrl2 = "/tep/staff/staffids";
  ...
  logout() {
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("/tep/staff/logout", {headers: headersSend})
      .map((data: Response)=> {
  ...
  onForgotPasswordSubmit(forgotPasswordData) {
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("/tep/staff/forgotpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
        console.log("sendLoginCredentials: Response ", data.json());
        return data.json();
        // return data['_body']
  ...
  sendResetPasswordCredentials(forgotPasswordCredentials){
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("/tep/staff/resetpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
        console.log("sendLoginCredentials: Response ", data.json());
        return data.json();
      });
  ...
  getUserName(){
    ...
    const headersSend = new Headers({'Content-Type':'application/json'});
    return this.http.get("/tep/staff/login",{headers: headersSend}).map(
      (res:Response)=> {

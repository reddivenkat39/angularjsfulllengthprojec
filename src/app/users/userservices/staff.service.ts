...
export class StaffService {
  ...
  sendLoginCredentials(staffLogin) {
    ...
    return this.http.post("http://tep/authorize/login", bodySend, {headers: headersSend})
      .map((data: Response)=> {
  ...
  sendToken(token) {
    let tokenUrl2 = "http://tep/authorize/staffids";
  ...
  logout() {
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("http://tep/authorize/logout", {headers: headersSend})
      .map((data: Response)=> {
  ...
  onForgotPasswordSubmit(forgotPasswordData) {
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("http://tep/authorize/forgotpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
        console.log("sendLoginCredentials: Response ", data.json());
        return data.json();
        // return data['_body']
  ...
  findEmailAddressByToken(token){
     let tokenUrl2 = "http://tep/findemailidbytoken";
    console.log('Bearer ' + token);
    const getHeaders = new Headers({'authorization': 'Bearer '+token});
  ...
  sendResetPasswordCredentials(forgotPasswordCredentials){
    ...
    const headersSend = new Headers({'Content-Type': 'application/json'});
    return this.http.post("http://tep/authorize/resetpassword", bodySend, {headers: headersSend})
      .map((data: Response)=> {
        console.log("sendLoginCredentials: Response ", data.json());
        return data.json();
      });
  ...
  getUserName(){
    ...
    const headersSend = new Headers({'Content-Type':'application/json'});
    return this.http.get("http://tep/authorize/login",{headers: headersSend}).map(
      (res:Response)=> {
